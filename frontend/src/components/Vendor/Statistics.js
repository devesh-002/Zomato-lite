    ...
    useEffect(() => {
        ...
        async function abc() {
            if (user_find !== null && type == "Vendor") {
                ...
                axios
                    .post("http://localhost:4000/Order/getvendororders", newUser)
                    .then((response) => {
                        ...
                        for (var i = 0; i < x.length; i++) {
                            if (x[i].status == "COMPLETED") {
                                countCompl++;
                                // axios
                                //     .post("http://localhost:4000/Buyer/viewprofile", { name: x[i].email })
                                //     .then((res) => {
                ...
                axios
                    .post("http://localhost:4000/Vendor/getfood", { canteen: user_find.ShopName })
                    .then((resp) => {
                        var x = []
    ...
    async function getGraphs() {
        ...
        for (var i = 0; i < orders.length; i++) {
            if (orders[i].status == "COMPLETED") {
                await axios
                    .post("http://localhost:4000/Buyer/viewprofile", { name: orders[i].email })
                    .then((res) => {
                        // setFoodItems(res.data)
                        batchStorer[res.data.batchName] = batchStorer[res.data.batchName] + 1;
                        if (res.data.age < 10) {
